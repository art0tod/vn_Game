...
define config.name = _("Симфония Смерти")
...
Да начнётся разработка!
Записал всех персонажей в файл сценария.
Дал название проекту.

------21.02.2022------

Начинаю писать сценарий.
Спрайтов пока нет, но вроде и так норм получается.
Впервые столкнулся с ошибко в файле options.rpy: какого-то хрена в нём переходы
должны обозначаться с маленькой буквы (надо запомнить)

------22.02.2022------

Активно пишется сценарий.

 пасивно ппишется сценарий------------------------------------------------------7
...
define build.name = "Symphony_of_Dead"
...
define config.sample_sound = "audio/sample-sound.mp3"
define config.sample_voice = "audio/sample-sound.mp3"
...
define config.save_directory = "vnGame-1645371233"
...
define config.window_icon = "gui/window_icon_2.png"
